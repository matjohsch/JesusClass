...
do while (maxDifference>tolerance)
    ...
    do cProd = 1,nGridProductivity        
        do cCapitalNext = 1,nGrid
            ...
            if (labourSteady) then
                Vendo(cCapitalNext,cProd)=log(mPolicyConsumption(cCapitalNext,cProd))-pphi/2.0*labourSteadyState**2.0+Vtilde(cCapitalNext,cProd)
            else
...
xtransformed = exp(x)
if (labourSteady) then
    f_endo = gridYendo(cCapitalNext,cProductivity) - vProductivity(cProductivity)*xtransformed**aalpha*labourSteadyState**(1.0-aalpha)-(1.0-ddelta)*xtransformed
else
